...
(async () => {
    ...
    ipfs = ipfsClent.create({ host: 'localhost', port: '5002', protocol: 'http' });
...
app.post("/", multer().array("files"), async (req, res) => {
    ...
    try {
        ...
        await Promise.all(req.files.map(async (file) => {
            //
            imgsData.push(await ipfs.add({ path: file.originalname, content: file.buffer }));
        }));
    ...
    // for await (const iterator of ipfs.addAll(globSource("./", "**/*.png"))) {
